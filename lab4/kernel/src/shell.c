...
struct CLI_CMDS cmd_list[CLI_MAX_CMD]=
{
    ...
    {.command="memory_testcase", .help="memory testcase generator, allocate and free"},
    {.command="simple_alloc", .help="simple allocator in heap session"},
    {.command="alloc", .help="dynamic alloc [SIZE] and free"},
...
void cli_cmd_exec(char* buffer)
{
    ...
    if (strcmp(cmd, "cat") == 0) {
        ...
        do_cmd_info();
    } else if (strcmp(cmd, "simple_alloc") == 0) {
        do_cmd_s_allocator();
    } else if (strcmp(cmd, "ls") == 0) {
        do_cmd_ls(argvs);
    } else if (strcmp(cmd, "memory_testcase") == 0) {
...
void do_alloc(char* argv)
{
    //int size = str_to_int(argv);
    unsigned int size = strtoul_custom(argv, NULL, 0);
    char *alloc = kmalloc(size);
    uart_sendline("%d\n",size);
    kfree(alloc);
}
...
void do_cmd_set2s()
{
    add_timer(timer_set2s,2,"2sAlert");
}
